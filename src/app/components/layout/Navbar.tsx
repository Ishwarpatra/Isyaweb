          ...
          <div className="px-4 py-4 flex flex-col gap-1">
            {navLinks.map((link) => (
              <Link
                ...
                className={`px-4 py-3 rounded-xl transition-all duration-200 text-[0.9rem] ${
                  isActive(link.path) ? "text-[#FFA500] bg-[#FFA500]/8 font-semibold" : "text-[#B0B8C1] font-normal"
                }`}
              >
                {link.label}
              </Link>
            ...
            <div className="flex flex-col gap-2 mt-3 pt-3 border-t border-white/6">
              <Link
                to="/login"
                onClick={closeMenu}
                className="px-4 py-3 rounded-xl text-center text-[#B0B8C1] text-[0.9rem] hover:text-white"
              >
                Sign In
              </Link>
              <Link
